...
A widely recognized limitation of molecular property prediction models is their reliance on chemical structures seen during training, resulting in poor generalization to out-of-distribution (OOD) compounds. In drug discovery, however, the most critical molecules often lie beyond the training distribution—making this bias particularly problematic. This mismatch introduces substantial **covariate shift**, under which standard deep learning models yield unstable and inaccurate predictions. The challenge is further compounded by the scarcity of labeled data, due to the high cost and complexity of experimental validation. 

To address these issues, we propose a novel **meta-learning-based approach** that leverages **unlabeled data** to interpolate between in-distribution (ID) and OOD samples, enabling the model to meta-learn robust generalization across distribution shifts. Our method demonstrates significant performance gains over state-of-the-art baselines on challenging real-world datasets exhibiting severe covariate shift.
