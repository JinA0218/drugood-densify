...
# validation_lists["strans_hivprot_count"]="3a4 hivint"
# validation_lists["strans_hivprot_bit"]="3a4 cb1"
# validation_lists["strans_dpp4_count"]="3a4 cb1"
# validation_lists["strans_dpp4_bit"]="3a4 cb1"
# validation_lists["strans_nk1_count"]="logd metab"
validation_lists["strans_nk1_bit"]="3a4 cb1"
    ...
    for vt in bit; do #  count
        for sencoder in "${sencoders[@]}"; do
            ...
            for ml in "${mixing_layers[@]}"; do
                ...
                MIXUP_EPOCHS=10 MIX_TYPE=MIXUP_BILEVEL RANDOM_YV=1 SAVE_TSNE_MODEL=1 MVALID_DEFAULT=1 MIXING_X_DEFAULT=$mix_default_type CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real_mixup.py \
                    --sencoder "$sencoder" \
                    --model mlp \
