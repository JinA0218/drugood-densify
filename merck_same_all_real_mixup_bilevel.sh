        ...
        for ((j=i+1; j<${#filtered[@]}; j++)); do
            ...
            for vt in count bit; do # count bit
                for sencoder in "${sencoders[@]}"; do
                    ...
                    MIXUP_EPOCHS=10 MIX_TYPE=MIXUP_BILEVEL RANDOM_YV=1 SAVE_TSNE_MODEL=0 MVALID_DEFAULT=MIXUP_BILEVEL4 CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real_mixup.py \
                        --sencoder "$sencoder" \
                        --model mlp \
                        --mixer_phi True \
                        ...
                        > logs/job_${job_id}_ds_${ds}_md_${md1}_${md2}_vt_${vt}_se_${sencoder}_MIXUP_BILEVEL4.log 2>&1 &
