...
validation_lists["dsets_hivprot_bit"]="cb1 thrombin"
validation_lists["dsets_dpp4_count"]="tdi thrombin"
validation_lists["dsets_dpp4_bit"]="tdi metab"
validation_lists["dsets_nk1_count"]="cb1 ox2"
validation_lists["dsets_nk1_bit"]="hivint tdi"

validation_lists["strans_hivprot_count"]="logd 3a4"
validation_lists["strans_hivprot_bit"]="3a4 ox2"
validation_lists["strans_dpp4_count"]="ox1 thrombin"
validation_lists["strans_dpp4_bit"]="ox1 rat_f"
validation_lists["strans_nk1_count"]="cb1 logd"
validation_lists["strans_nk1_bit"]="3a4 ox2"
...
for ds in "${targets[@]}"; do
    for vt in count bit; do
        for sencoder in "${sencoders[@]}"; do
            ...
            # Build contexts: other datasets except current ds
            other_targets=()
            for t in "${targets[@]}"; do
                if [[ "$t" != "$ds" ]]; then
                    other_targets+=("$t")
            ...
            # contexts=("${other_targets[0]}" "${other_targets[1]}" "${other_targets[0]} ${other_targets[1]}")

            # for context in "${contexts[@]}"; do
            gpu_id=${gpus[$((job_id % gpu_count))]}

            echo "Launching job $job_id on GPU $gpu_id | ds=$ds | md=($md1,$md2) | vt=$vt | sencoder=$sencoder | context=($context)"
            ...
            CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real.py \
                ...
                --same_setting \
                --mvalid_dataset "$md1" "$md2" \
                --embed_test base_cX_mO \
                > logs/job_${job_id}_ds_${ds}_md_${md1}_${md2}_vt_${vt}_se_${sencoder}_context_${context// /_}.log 2>&1 &
