...
class Merck(Dataset):
    def __init__(self, split="train", vec_type='count', dataset="hivprot", is_context=False, batchsize=64, mvalid_dataset=None):
...
def get_dataset(args, test=False):
    ...
    if args.num_inner_dataset == 1:
        if args.same_setting:
            ...
            trainset = Merck(split="train", vec_type=args.vec_type, dataset=args.dataset, is_context=False, mvalid_dataset=args.mvalid_dataset) # hivprot
            ...
            if test:
                ...
                validset = Merck(split="train", vec_type=args.vec_type, dataset=args.dataset, is_context=False, mvalid_dataset=args.mvalid_dataset)
            ...
            testset = Merck(split="test", vec_type=args.vec_type, dataset=args.dataset, is_context=False, mvalid_dataset=args.mvalid_dataset) # hivprot

            # validset for outer loop
            mvalidset = None
            print('MVALID ', args.mvalid_dataset)
            for dataset in args.mvalid_dataset:
                _validset = Merck(split="train", vec_type=args.vec_type, dataset=dataset, is_context=False, mvalid_dataset=args.mvalid_dataset) # "dpp4", "nk1"
            ...
            trainset = Merck(split="train", vec_type=args.vec_type, dataset=args.dataset, is_context=False, mvalid_dataset=args.mvalid_dataset)
            ...
            testset = Merck(split="test", vec_type=args.vec_type, dataset=args.dataset, is_context=False, mvalid_dataset=args.mvalid_dataset)

            # validset for outer loop
            mvalidset = None
            for dataset in [d for d in ["hivprot", "dpp4", "nk1"] if d != args.dataset]:
                _validset = Merck(split="train", vec_type=args.vec_type, dataset=dataset, is_context=False, mvalid_dataset=args.mvalid_dataset)
                if mvalidset is None:
                    mvalidset = _validset
                    continue
        ...
        trainset = None
        for dataset in [d for d in ["hivprot", "dpp4", "nk1"] if d != args.dataset]:
            _trainset = Merck(split="train", vec_type=args.vec_type, dataset=dataset, is_context=False, mvalid_dataset=args.mvalid_dataset)
            if trainset is None:
                trainset = _trainset
                continue
        ...
        mvalidset = Merck(split="train", vec_type=args.vec_type, dataset=args.dataset, is_context=False, mvalid_dataset=args.mvalid_dataset)
    ...
    if args.mixer_phi:
        ...
        contextset = Merck(split="train", vec_type=args.vec_type, dataset=args.dataset, is_context=True, mvalid_dataset=args.mvalid_dataset)
