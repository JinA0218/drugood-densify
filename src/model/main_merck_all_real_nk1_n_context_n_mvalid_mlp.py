...
class Trainer:
    ...
    def train(self, mvalidloader):
        ...
        for x, y in self.trainloader:
            ...
            loss_model = F.mse_loss(y_hat[0].squeeze(), y.to(self.args.device))
...
if __name__ == '__main__':
    ...
    if os.environ.get("HYPER_SWEEP", "0") == "1":
        ...
        # else:
        path = f"experiments5/hyper_search_{args.sencoder}_n_mvalid_real_ryv{os.environ.get('RANDOM_YV', '0')}_mlp_real2"
        os.makedirs(path, exist_ok=True)
    ...
    os.makedirs("./experiments5", exist_ok=True)
    if not args.same_setting:
        ...
        if not args.mixer_phi:
            with open(f"./experiments5/results-{args.sencoder}-MLP_real2.txt", "a+") as _f:
            ...
            if args.context_dataset != None:
                ...
                if os.environ.get('RANDOM_YV', '0') == '0':
                    ...
                    if os.environ.get('MIX_TYPE', 'SET') == 'SET':
                        ...
                        with open(f"./experiments5/S-results-{args.sencoder}_mvalid-all-3real-ml{args.mixing_layer}-{os.environ.get('MIXING_X_DEFAULT', 'xmix')}-mvdef{os.environ.get('MVALID_DEFAULT', '1')}-mNct{args.model_no_context}-RYV{os.environ.get('RANDOM_YV', '0')}_{os.environ.get('MIX_TYPE', 'SET')}_real_3.txt", "a+") as _f:
