...
dataset_names=('3a4' 'cb1' 'hivint' 'logd' 'ox1') #  'ox2' 'pgp' 'ppb' 'rat_f' 'tdi' 'thrombin'
...
for ds in "${targets[@]}"; do
    ...
    for ((i=0; i<${#filtered[@]}-1; i++)); do
        for ((j=i+1; j<${#filtered[@]}; j++)); do
            ...
            # Skip specific pair
            # if [[ "$md1" == "ox2" && "$md2" == "thrombin" ]]; then
            for vt in count bit; do # count bit
                for sencoder in "${sencoders[@]}"; do
                    gpu_id=${gpus[$((job_id % gpu_count))]}

                    echo "Launching job $job_id on GPU $gpu_id | ds=$ds | md=($md1,$md2) | vt=$vt | sencoder=$sencoder"

                    RANDOM_YV=1 SAVE_TSNE_MODEL=0 MVALID_DEFAULT=NO_SEED_RAND_PAIR2 CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real.py \
                        --sencoder "$sencoder" \
                        --model mlp \
                        --mixer_phi True \
                        --optimizer adamwschedulefree \
                        ...
                        --same_setting \
                        --mvalid_dataset "$md1" "$md2" \
                        > logs/job_${job_id}_ds_${ds}_md_${md1}_${md2}_vt_${vt}_se_${sencoder}_noseed_rand.log 2>&1 &
