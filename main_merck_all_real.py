...
class Trainer:
    ...
    def train(self):
        # raise NotImplementedError() # implement this and check we're getting mvalid in the same order 
        losses = []
        self.model.train()
        ...
        # for x, y in self.trainloader:
            ...
            y_hat = y_hat[0]
            # print('y_hat ', y_hat)
            # print('y_hat_0 ', y_hat[0])
            # print('y_hat_0 shape ', y_hat[0].shape)
        ...
        #         y_hat, _, _ = self.model(x=x.to(self.args.device))
        ...
        # for x, y in self.trainloader:
        #     y_hat, _, _ = self.model(x=x.to(self.args.device), embedding_list=None, label_list=None, embed_type=None)
        #     loss = F.mse_loss(y_hat.squeeze(), y.to(self.args.device))
        #     losses.append(loss.item())
        #     loss.backward()
        #     self.optimizer.step()
    ...
    def train_mixup(self):
        ...
        for episode in tqdm(range(self.args.outer_episodes * self.args.inner_episodes), ncols=75, leave=False):
            if episode % self.args.inner_episodes == 0:
                losses = []
            ...
            if episode!=0 and episode % self.args.inner_episodes == 0:
                vmse = self.test(dataloader=self.validloader, mixer_phi=self.mixer_phi)

                print('Episode: {:<3} tloss: {:.3f} vmse: {:.3f}'.format(\
                    episode//self.args.inner_episodes, np.mean(losses), vmse))
    ...
    def fit(self):
        ...
        if self.mixer_phi is None:
            ...
            print('tloss: {:.3f} vmse: {:.3f}'.format(tloss, vmse))
        ...
        if os.environ.get('SAVE_TSNE_MODEL', '0') == '1':
            ...
            if args.mixer_phi:
                torch.save({
                    ...
                    'vmse' : vmse,
                    'tmse ': tmse, 
                }, f"/c2/jinakim/Drug_Discovery_j/tsne_model2_mNct{args.model_no_context}_RYV{os.environ.get('RANDOM_YV', '0')}_mix{args.mixer_phi}/{self.args.embed_test}/Model_{args.sencoder}_{args.dataset}_{args.vec_type}_{args.mvalid_dataset}.pth")
            else:
                torch.save({
                    'model': self.model.state_dict(),
                    'optimizer' : self.optimizer.state_dict(),
            ...
            print(f'saved >> model_{args.sencoder}_{args.dataset}_{args.vec_type}_{args.mvalid_dataset}.pth')
            exit()
        ######
