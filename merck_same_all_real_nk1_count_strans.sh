            ...
            for sencoder in "${sencoders[@]}"; do
                ...
                for ((i=0; i<${#filtered[@]}-1; i++)); do
                    for ((j=i+1; j<${#filtered[@]}; j++)); do
                        ...
                        RANDOM_YV=1 SAVE_TSNE_MODEL=0 MVALID_DEFAULT=nkq_count_2strans CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real.py \
                            --sencoder "$sencoder" \
                            ...
                            > logs/job_${job_id}_ds_${ds}_md_${md1}_${md2}_vt_${vt}_se_${sencoder}_sl_${sencoder_layer}_nkq_count_2strans.log 2>&1 &
