...
# TODO:
# - make max input features 6561 and downproject
...
class Trainer:
    ...
    def test(self, dataloader, mixer_phi=None):
        ...
        with torch.no_grad():
            ...
            for x, y in dataloader:
                ...
                loss = self.calc_loss(y_hat.cuda().squeeze(), y.cuda().squeeze(), test=True)
                losses.append(loss.item())
    ...
    def train_mixer_phi(self):
        ...
        def train_mixer(model, optimizer, mixer_phi, x, y, context, device, interp_loss=False):
            ...
            # y = self.trainloader.dataset.denormalize(y)
        ...
        for episode in tqdm(range(self.args.outer_episodes), ncols=75, leave=False):
            ...
            y_v = self.trainloader.dataset.dataset.denormalize(y_v)
    ...
    def fit(self):
        ...
        print('(Last Model) Tmse: {:.3f}'.format(tmse))
        ...
        tmse = self.test(dataloader=self.testloader, mixer_phi=self.mixer_phi)
        print('(Best MSE)   Tmse: {:.3f}'.format(tmse))
