    ...
    for vt in bit; do #  count
        for sencoder in "${sencoders[@]}"; do
            ...
            for ml in "${mixing_layers[@]}"; do
                ...
                RANDOM_YV=1 SAVE_TSNE_MODEL=1 MVALID_DEFAULT=tsne_best_origin MIXING_X_DEFAULT=$mix_default_type CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real.py \
                    --sencoder "$sencoder" \
                    --model mlp \
                    --optimizer adamwschedulefree \
                    --seed 42 \
