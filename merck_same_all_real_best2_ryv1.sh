...
validation_lists["dsets_nk1_bit"]="hivint tdi"
...
# NOTE main_merck_all_real changed to change the senc_layer
sencoder_layers=("pma") #  "max" "mean" "sum"
...
for ds in "${targets[@]}"; do
    for vt in count bit; do # bit
        for sencoder in "${sencoders[@]}"; do
            ...
            for sencoder_layer in "${sencoder_layers[@]}"; do
                for ml in "${mixing_layers[@]}"; do

                    gpu_id=${gpus[$((job_id % gpu_count))]}

                    echo "Launching job $job_id on GPU $gpu_id | ds=$ds | md=($md1,$md2) | vt=$vt | sencoder=$sencoder | sencoder_layer=$sencoder_layer | mixing_layer=$ml"
                    mix_default_type="xmix"
                    RANDOM_YV=1 SAVE_TSNE_MODEL=0 MVALID_DEFAULT=improve_NK1_REAL MIXING_X_DEFAULT=$mix_default_type CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real.py \
                        --sencoder "$sencoder" \
                        ...
                        --mixer_phi True \
                        --mixing_layer "$ml" \
                        > logs/job_${job_id}_ds_${ds}_md_${md1}_${md2}_vt_${vt}_se_${sencoder}_sl_${sencoder_layer}_mt_${mix_default_type}_ml_${ml// /_}_improve_NK1_REAL.log 2>&1 &
...
#                 mix_default_type="xmix"
#                 RANDOM_YV=1 SAVE_TSNE_MODEL=0 MVALID_DEFAULT=IMPROVE_NK1 MIXING_X_DEFAULT=$mix_default_type CUDA_VISIBLE_DEVICES=$gpu_id PYTHONPATH=. python main_merck_all_real.py \
#                     --sencoder "$sencoder" \
#                     --model mlp \
#                     --optimizer adamwschedulefree \
#                     --seed 45 \
